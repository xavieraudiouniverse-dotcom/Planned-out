...
export function CollaborativeCalendar({
  ...
  const refreshAccess = useCallback(async () => {
    ...
    const [calendarResult, memberResult, inviteResult] = await Promise.all([
  ...
  const refreshEvents = useCallback(async () => {
    ...
    const { data, error } = await sb
      ...
      .gte('starts_at', firstVisible.toISOString())
  ...
  async function sendInvite(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    const { error } = await sb.from('xp_calendar_invites').insert({
      calendar_id: activeCalendar.id,
      owner_id: user.id,
    ...
    setInviteEmail('');
    await refreshAccess();
    notify('Invitation created. They will see it when signed in with that email.');
  }
  ...
  const sharedEventsForDay = (date: string) => events.filter((event) => localDateKey(new Date(event.starts_at)) === date);
